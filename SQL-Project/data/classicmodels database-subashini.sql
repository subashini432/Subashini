...
/*Q1.a.Fetch the employee number, first name and last name of those employees 
who are working as Sales Rep reporting to employee with employeenumber 1102 */
...
/*Q2.a. Using a CASE statement, segment customers into three categories based on their country:(Refer Customers table)
...
select productcode,sum(quantityordered)as total_ordered from orderdetails group by productcode ORDER BY total_ordered DESC
LIMIT 10;

/* Q3.b.Company wants to analyse payment frequency by month. Extract the month name from the payment date to count the 
total number of payments for each month and include only those months with a payment count exceeding 20. 
Sort the results by total number of payments in descending order.  (Refer Payments table). */
select * from payments;
select monthname(paymentDate) as Payment_month , count(amount)as Num_payments from payments group by Payment_month having count(amount)>20;
...
use classicmodels;
select country,count(ordernumber) as order_count from customers as c inner join
orders as o where c.customernumber=o.customernumber group by country order by order_count desc limit 5;
...
CREATE TABLE project (
    ...
    insert into project values(1,"pranaya","male",3),
                           ...
                           (7,"hina","female",3);
    select * from project;
    select M.fullname as Managername,E.fullname as Employeename from project M join project E 
    on M.EmployeeId=E.ManagerId order by M.fullname;
...
/* Q10.a) Window functions - Rank, dense_rank, lead and lag
...
/*  Q10. b) Calculate year wise, month name wise count of orders and year over year (YoY) 
...
WITH MonthlyOrders AS (
    SELECT
        ...
        COUNT(*) AS OrderCount
    FROM orders
    GROUP BY OrderYear, MonthNum, OrderMonth
)
SELECT
    mo.OrderYear,
    mo.OrderMonth,
    ...
    CONCAT(
        IFNULL(
            ROUND(
                100.0 * (mo.OrderCount - LAG(mo.OrderCount) OVER 
                         (PARTITION BY mo.OrderYear ORDER BY mo.MonthNum)) /
                LAG(mo.OrderCount) OVER (PARTITION BY mo.OrderYear ORDER BY mo.MonthNum),
                0
            ),
            0
        ),
...
ORDER BY mo.OrderYear, mo.MonthNum;


/* Q11.Subqueries and their applications
a. Find out how many product lines are there for which the buy price value
 is greater than the average of buy price value. Show the output as product line and its count. */
select * from products;
Select P.ProductLine,count(*) as Total from Products P join (select avg(buyPrice) as avgs from products) as averages on P.buyPrice > averages.avgs
group by productline order by Total desc;

--  Q12. ERROR HANDLING in SQL
Create table Emp_EH ( emp_id int primary key,emp_name varchar(30),email_address varchar(30));
call classicmodels.Insert_Employee_EH(1, 'suba', 'suba@example.com');
...
('Robinonso', 'Scientist', '2020-10-04', -2);
